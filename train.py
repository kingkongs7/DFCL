...
def prepare_training_data(config):
    ...
    if 'dataset_type' in config and config['dataset_type'] == 'blend':
        ...
        elif config['model_name'] == 'sbi' or config['model_name'] == 'clip3':
    ...
    elif 'dataset_type' in config and config['dataset_type'] == 'trans':
        train_set = TransDataset(config, mode='train')
    elif 'dataset_type' in config and config['dataset_type'] == 'bian':
        train_set = BianDataset(config, mode='train')
    elif 'dataset_type' in config and config['dataset_type'] == 'slide':
        train_set = SlideDataset(config, mode='train')
    else:
    ...
    elif config.get('sampler', None) == 'slide':
        from dataset.slide import SlideSampler
        # custom_sampler = SlideSampler(
        #     label_list=train_set.label_list,
        #     stats_list=train_set.stats_list, 
        #     K=config['K'], total_epochs=config['nEpochs'],
    ...
    return train_set, train_data_loader
...
def prepare_testing_data(config):
    def get_test_data_loader(config, test_name):
        ...
        if config.get('dataset_type', None) == 'trans':
            test_set = TransDataset(config, 'test')
        elif not config.get('dataset_type', None) == 'lrl':
...
def main():
    ...
    if config['lmdb']:
        config['dataset_json_folder'] = '/data/yyl/data/deepfake_dataset/config/dataset_json'
    # create logger
    ...
    train_dataset, train_data_loader = prepare_training_data(config)
    ...
    for epoch in range(config['start_epoch'], config['nEpochs'] + 1):
        if config.get('sampler', None) == 'slide':
            losses = trainer.get_losses(train_dataset=train_dataset, config=config)
            # train_data_loader.sampler.set_epoch(epoch)
            train_data_loader.sampler.update_state(epoch, losses)
